...
export const authUser = (fields, role, mode) => async (dispatch) => {
    ...
    try {
      //  alert(`${process.env.REACT_APP_BASE_URL}/${role}${mode}`)
        const result = await axios.post(`http://localhost:3000/CustomerRegister`, fields, {
...
export const addStuff = (address, fields) => async (dispatch) => {
    ...
    try {
        // alert(`${process.env.REACT_APP_BASE_URL}/${address}`)
        const result = await axios.post(`http://localhost:3000/ProductCreate`, fields, {
            headers: { 'Content-Type': 'application/json' },
        });
